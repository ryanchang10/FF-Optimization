...
# The test statistic for this test based on this model is high, and the p-value lies just within our significance level. We fail to reject the null hypothesis that the residuals of this model are not normally distributed, allowing us to proceed assuming unbiased and efficient predictors (minimum variance, and expected value = true value).
```


```{r}
# Web scrape 2024-2025 season predictions via fantasypros.com
...
# WOW! The difference between Christian McCaffrey's projected points and the next best RB's (Breece Hall) is 46.2!!!
```
...
# Interesting...the difference between CeeDee Lamb and Tyreek's predicted points is almost nonexistent, but the dropoff after Tyreek to Ja'Marr Chase is huge! This tells me that if you have the chance to draft CeeDee or Tyreek, you shouldn't wait to take them. 
```
...
# While these tests for normality 'fail', I want to keep the outliers because the TE position is quite skewed. Historically, there are only a few TE that produce lots of fantasy points (Kelce, Mark Andrews, etc), and the dropoff to the average TE is large. I will proceed knowing results may be unbiased and inefficient due to this. 
```
...
# The dropoff between each tight end is relatively steady, with about a 4-6 point dropoff per...those could add up quick, though, so prioritizing TE is important. 
```


```{r}
# Exporting to use in Python
write.csv(qb_predictions,"C:\\users\\Ryan Chang\\downloads\\qb_predictions.csv", row.names = FALSE)
write.csv(rb_predictions,"C:\\users\\Ryan Chang\\downloads\\rb_predictions.csv", row.names = FALSE)
write.csv(te_predictions,"C:\\users\\Ryan Chang\\downloads\\te_predictions.csv", row.names = FALSE)
write.csv(wr_predictions,"C:\\users\\Ryan Chang\\downloads\\wr_predictions.csv", row.names = FALSE)
```
